...
Die Sentry-DSN liegt ebenfalls nicht im Git-Repo. Lokal kommt sie aus `MigraineTracker/Configs/LocalSecrets.xcconfig`; in Xcode Cloud wird dieselbe Datei vor dem Build aus der Secret-Variable `SENTRY_DSN` erzeugt.
...
- `ci_pre_xcodebuild.sh` erzwingt die Release-Regeln des Projekts:
  ...
  - erzeugt `MigraineTracker/Configs/LocalSecrets.xcconfig` aus dem Secret `SENTRY_DSN`
